...
const runImportWorkflowSmoke = async (browser) => {
  ...
  const rocketSourcePanelText = await textOf(page.locator(".panel-svg-source"));
  await openTargetsPanel(page);
  ...
  assert(rocketSourcePanelText.includes("0 tracks kept, 3 removed"), "paste import did not reconcile sample tracks");
  assert(
  ...
  const badgeSourcePanelText = await textOf(page.locator(".panel-svg-source"));
  await openTargetsPanel(page);
  ...
  assert(badgeSourcePanelText.includes("0 tracks kept, 1 removed"), "file upload did not reconcile the imported Flame track");
  ...
  const resetSourcePanelText = await textOf(page.locator(".panel-svg-source"));
  await openTargetsPanel(page);
  ...
  assert(resetSourcePanelText.includes("Restored 3 sample tracks"), "reset sample status did not report restored tracks");
