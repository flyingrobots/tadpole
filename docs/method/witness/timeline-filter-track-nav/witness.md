...
- UI
  ...
  - `Show selected track only` still enforces single-track visibility after filtering.
  - Status chip updates to the current visible track count.
- Keyframes
  - `,` and `.` are wired in global keyboard handler.
  - Toolbar buttons for Prev/Next keyframe appear and are disabled when no selected track keyframes exist.
  - Jump actions move playhead to a keyframe on the selected track and update selected keyframe.
