...
- `BEARING.md` and `CHANGELOG.md` exist and include current state, priorities, and unreleased notes.
- `README.md` includes branch/PR METHOD process note.
- `docs/method/backlog/inbox/` contains `.gitkeep` and a dated signpost intake note.

## Acceptance
If all checks pass, this cycle's ship-surface deliverable is reproducible from repo state.
